...
I need send messages in slack channel when i deploy things, ou backups and restore dbs, so with this script i can keep the entire team updated when certain actions hapen.
...
```
Copy this content and create a file called slack-cli and give him permission to execute.

```terminal
chomd +x slack-cli
```

Generate a token in Slack app and put him into a file .slack in same directory of slack-cli
